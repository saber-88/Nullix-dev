...
// #include "sys_proc/SubProcess.hpp"
...
//         return SubProcess(k_ref_str_binName_ , k_ref_vecStr_argv_).mt_Res_execute();
    ...
    ]]  uniqx
{
    ...
    -> Result_t
    {
        ...
        else
        {
                ...
                ( !_outPipeDes_RX.closed ( ) || !_errPipeDes_RX.closed ( ) )
            {
                ...
                if
                    ( hasFlag( pOpt_IO_ & ProcOpt::stdout ) )
                {
    ...
    -> int
    {
        ...
        return
            (
                this->SubProcess::mt_Res_execute
                ( PM_pOpt_opt ).exit_code ( )
            )
        ;
